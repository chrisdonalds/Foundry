...
class ImageManipulation {
	...
	public function ImageManipulation($imgfile){
		//detect image format
		$this->image["format"] = preg_replace("/.*\.(.*)$/i", "\\1", $imgfile);
		...
		//list($this->image["width"], $this->image["height"]) = getImageSize($this->image["src"]);
	}
	...
	public function setImageHeight($height=100){
		//height
		$this->image["height_thumb"] = $height;
		$this->image["width_thumb"] = ($height/$this->image["height"])*$this->image["width"];
	}
	...
	public function setImageWidth($width=100){
		//width
		$this->image["width_thumb"] = $width;
		$this->image["height_thumb"] = ($width/$this->image["width"])*$this->image["height"];
	}
	...
	public function resize($size=100){
		if ($this->image["width"] >= $this->image["height"]) {
			$this->image["width_thumb"] = $size;
			$this->image["height_thumb"] = round(($size/$this->image["width"])*$this->image["height"]);
		} else {
			$this->image["height_thumb"] = $size;
			$this->image["width_thumb"] = round(($size/$this->image["height"])*$this->image["width"]);
		}
	}
	...
	private function createResampledImage(){
		...
		if (isset($this->image["width_thumb"]) && isset($this->image["height_thumb"])) {
			$this->image["des"] = ImageCreateTrueColor($this->image["width_thumb"], $this->image["height_thumb"]);
			imagecopyresampled($this->image["des"], $this->image["src"], 0, 0, $this->image["srcx"], $this->image["srcy"], $this->image["width_thumb"], $this->image["height_thumb"], $this->image["width"], $this->image["height"]);
		} else {
			$this->image["des"] = ImageCreateTrueColor($this->image["width"], $this->image["height"]);
			imagecopyresampled($this->image["des"], $this->image["src"], 0, 0, $this->image["srcx"], $this->image["srcy"], $this->image["width"], $this->image["height"], $this->image["width"], $this->image["height"]);
		}
	}
	...
	public function save($save=""){
		...
		if ($this->image["format"] == "JPG" || $this->image["format"] == "JPEG") {
			//JPEG
			$ok = imageJPEG($this->image["des"], $save, $this->image["quality"]);
		} elseif ($this->image["format"] == "PNG") {
			//PNG
			$ok = imagePNG($this->image["des"], $save);
		} elseif ($this->image["format"] == "GIF") {
			//GIF
			$ok = imageGIF($this->image["des"], $save);
		} elseif ($this->image["format"] == "WBMP") {
			//WBMP
			$ok = imageWBMP($this->image["des"], $save);
		}
		return $ok;
	}
}