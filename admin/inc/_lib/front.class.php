...
/**
 * RPCCLASS
 * Stores and manages RSS, ATOM and XMLRPC data
 */
class RPCClass {
	// overloaded data
	private $_rss = array();
	private $_keys = array(	"table", "childtable", "id" );
	private $_arykeys= array();
	private $_subkeys = array( "destfile", "queryvars", "category", "dbrec",
                               "found", "error", "datefrom", "dateto",
                               "titlefld", "contentfld", "datefld",
                               "numrows");
	protected static $_instance = null;
	...
	public function show_properties(){
        foreach($this->_keys as $k) print "[$k] = ".getIfSet($this->_rss[$k])."<br>";
        foreach($this->_arykeys as $k)
            if(isset($this->_rss[$k])) print "[$k] = <pre>".printr($this->_rss[$k])."</pre><br/>";
        foreach($this->_subkeys as $k){
            if(isset($this->_rss['subkeys'][$k])) {
                print "[$k] = ";
                if(is_array($this->_rss['subkeys'][$k]))
                    print "<pre>".print_r($this->_rss['subkeys'][$k], true)."</pre><br/>";
                else
                    print $this->_rss['subkeys'][$k]."<br/>";
            }
        }
	}
	...
	public function __get($name){
		if(in_array($name, $this->_keys)){
			// return scalar value
			return getIfSet($this->_rss[$name]);
		}elseif(in_array($name, $this->_arykeys)){
			// return array
			return getIfSet($this->_rss[$name]);
		}elseif(in_array($name, $this->_subkeys)){
			// return subarray value
			return getIfSet($this->_rss['subkeys'][$name]);
		}else{
			trigger_error("Cannot get '$name'.  It is not a valid _RPC property. ");
		}
	}
	...
	public function __set($name, $value){
		if(in_array($name, $this->_keys)){
			// set scalar value
			//$this->checkCaller("set");
			$this->_rss[$name] = $value;
		}elseif(in_array($name, $this->_arykeys)){
			// set array
			$this->_rss[$name][] = $value;
		}elseif(in_array($name, $this->_subkeys)){
			// set subarray value
			$this->_rss['subkeys'][$name] = $value;
		}else{
			trigger_error("Cannot set '$name'.  It is not a valid _DATA property. ");
		}
	}
